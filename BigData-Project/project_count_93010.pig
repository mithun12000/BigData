...
group_count = group grouped_filtered_medicare all;
group_count_final = foreach group_count GENERATE COUNT(grouped_filtered_medicare);
STORE group_count_final INTO '/user/shavanur/output1' USING org.apache.pig.piggybank.storage.CSVExcelStorage('|','NO_MULTILINE','WINDOWS');
...
group_physician_count = group grouped_physician all;
group_phy_count_final = foreach group_physician_count GENERATE COUNT(grouped_physician);
STORE group_phy_count_final INTO '/user/shavanur/output2' USING org.apache.pig.piggybank.storage.CSVExcelStorage('|','NO_MULTILINE','WINDOWS');
...
final_count = group final_records all;
final_count_records = foreach final_count GENERATE COUNT(final_records);
STORE final_count_records INTO '/user/shavanur/output3' USING org.apache.pig.piggybank.storage.CSVExcelStorage('|','NO_MULTILINE','WINDOWS');
